...
- нейропомощник: отвечает на вопросы о номерах, условиях проживания и бронировании с помощью локальной LLM через Ollama
...
Проект разделён на пользовательскую часть (Telegram-бот) и административную часть (Django Admin), с централизованной базой данных и отдельными сервисами для бронирования, каталога и NLP-обработки.

```mermaid
graph TD
    subgraph "👨‍💼 Django Admin"
        A[Django Admin Panel] --> B[Управление номерами]
        A --> C[Управление бронированиями]
        A --> D[Модерация отзывов]
    ...
    subgraph "👤 Telegram Bot"
        U[Пользователь] --> E[Каталог номеров]
        U --> F[Бронирование]
        U --> G[Отзывы]
        U --> H[Чат-помощник]
        E --> PostgreSQL
